	...
	persist(
		(set, get) => ({
			...
			setAllContent: (contentData) => {
				...
				const transformedData = transformParsedResumeToSchema(contentData, get().portfolioType);
				set(() => ({
			...
			setParsedData: (data) => {
				const transformedData = transformParsedResumeToSchema(data, get().portfolioType);
				set(() => ({
					parsedData: data,
			...
			restoreFromParsed: () => {
				...
				if (state.parsedData && Object.keys(state.content).length === 0) {
					const transformedData = transformParsedResumeToSchema(state.parsedData, state.portfolioType);
					set(() => ({
