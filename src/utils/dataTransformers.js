...
import {
	EMPTY_PORTFOLIO,
	PORTFOLIO_SCHEMA,
} from "@/data/schemas/portfolioSchema";
 ...
 */
export const transformParsedResumeToSchema = (
	parsedData,
	...
	try {
		...
		if (parsedData.education?.degrees) {
			transformed.education.degrees = parsedData.education.degrees.map(
				(degree) => ({
					id: generateId(),
		...
		if (parsedData.skills) {
			...
			if (parsedData.skills.languages) {
				transformed.skills.languages = parsedData.skills.languages.map(
					(lang) => ({
						name:
							typeof lang === "string"
								? lang.split("(")[0].trim()
		...
		if (parsedData.achievements) {
			if (parsedData.achievements.awards) {
				transformed.achievements.awards = parsedData.achievements.awards.map(
					(award) => ({
						id: generateId(),
			...
			if (parsedData.achievements.certifications) {
				transformed.achievements.certifications =
					parsedData.achievements.certifications.map((cert) => ({
						id: generateId(),
			...
			if (parsedData.achievements.publications) {
				transformed.achievements.publications =
					parsedData.achievements.publications.map((pub) => ({
						id: generateId(),
	...
	try {
		...
		if (oldData.contact) {
			...
			if (oldData.contact.location) {
				const locationParts = oldData.contact.location
					.split(",")
					.map((p) => p.trim());
				transformed.personal.location.city = locationParts[0] || "";
				transformed.personal.location.state = locationParts[1] || "";
		...
		if (oldData.education?.degrees) {
			transformed.education.degrees = oldData.education.degrees.map(
				(degree) => ({
					id: generateId(),
					institution: degree.institution || "",
					degree: degree.degree || "",
		...
		if (oldData.showcase || oldData.projects) {
			...
			if (typeof projectsData.projects === "string" && projectsData.projects) {
				...
				transformed.projects.items = projectsData.projects
					.split(",")
					.map((proj) => ({
						id: generateId(),
						title: proj.trim(),
		...
		if (oldData.achievements) {
			if (oldData.achievements.awards) {
				transformed.achievements.awards = oldData.achievements.awards.map(
					(award) => ({
						id: generateId(),
						title: award,
						organization: "",
			...
			if (oldData.achievements.certifications) {
				transformed.achievements.certifications =
					oldData.achievements.certifications.map((cert) => ({
						id: generateId(),
						name: cert,
						organization: "",
						issueDate: "",
			...
			if (oldData.achievements.publications) {
				transformed.achievements.publications =
					oldData.achievements.publications.map((pub) => ({
						id: generateId(),
						title: pub,
						type: "article",
						publisher: "",
...
export const transformSchemaToComponentProps = (schemaData, section) => {
	...
	try {
		switch (section) {
			...
			case "achievements":
				return {
					...
					certifications:
						schemaData.achievements?.certifications?.map((c) => c.name) || [],
					publications:
						schemaData.achievements?.publications?.map((p) => p.title) || [],
					data: schemaData,
				};
		...
		console.error(
			`Error transforming schema to component props for section ${section}:`,
			error
		);
		return { data: schemaData };
	}
...
export const validatePortfolioData = (data) => {
	...
	try {
		...
		if (!data.experience?.jobs || data.experience.jobs.length === 0) {
			warnings.push(
				"Adding work experience will make your portfolio more compelling"
			);
		}

		if (!data.skills?.technical || data.skills.technical.length === 0) {
			warnings.push(
				"Adding technical skills will help showcase your expertise"
			);
		}

		if (!data.projects?.items || data.projects.items.length === 0) {
			warnings.push(
				"Adding projects will demonstrate your practical experience"
			);
		}
...
const calculateCompleteness = (data) => {
	const sections = [
		...
		{ section: "about", weight: 15, complete: !!data.about?.summary },
		{
			section: "experience",
			weight: 20,
			complete: !!(data.experience?.jobs && data.experience.jobs.length > 0),
		},
		{
			section: "skills",
			weight: 15,
			complete: !!(data.skills?.technical && data.skills.technical.length > 0),
		},
		{
			section: "projects",
			weight: 20,
			complete: !!(data.projects?.items && data.projects.items.length > 0),
		},
		{
			section: "contact",
			weight: 10,
			complete: !!(data.personal?.email || data.contact?.email),
		},
	];
